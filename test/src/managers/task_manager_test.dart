...
void main() {
  ...
  group('TaskManager - resolvers', () {
    ...
    test('TaskManager - executeTask in TaskPool mode', () async {
      ...
      expect(
          () =>
              extraFuture = taskManager.executeTaskInPool(
                ...
                taskId: '1',
                task: (status) => Future
                    .delayed(taskDuration * 2)
                    .then((value) => poolSize + 1),
              ),
          returnsNormally,
      );

      final currentTaskQueue = taskManager
          .taskResolvers
          .entries
          .first
          .value
      ...
      int countingTasksInExecution() {
        final identified =  currentTaskQueue
          .map((c) => c.isExecuting ? 1 : 0);
    ...
    test('TaskManager - executeTask in rejectAfterThreshold mode', () async {
      final taskManager = AwesomeTaskManager().createRejectedAfterThresholdManager();
  ...
  group('resetManager', () {
    test('reset should clear all internal list controls', () {
      final taskManager = AwesomeTaskManager().createSharedResultManager();
      final result = taskManager
          .executeTaskSharingResult<String>(
